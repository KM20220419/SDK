...
namespace io::gripper
{
    ...
    struct DeviceProfile
    {
        ...
        bool release_torque_on_disconnect = true; // 3次通信失败：true: 释放力矩 (默认), false: 保持力矩并停止
    ...
    {
    public:
        explicit GripperDriver(std::string port, std::optional<DeviceProfile> profile = std::nullopt, std::string config_file_path_ = "");
        ~GripperDriver();
        ...
        bool commandPosition(uint8_t servo_id,
                             std::optional<uint16_t> position_raw,
                             std::optional<float> angle_rad);

        bool commandVelocity(uint8_t servo_id,
                             std::optional<uint16_t> velocity_raw,
                             std::optional<float> velocity_rad_s,
                             std::optional<int> move_time_ms);

        bool setEffortLimit(uint8_t servo_id,
                            std::optional<float> current_limit_mA,
        ...
        std::vector<GripperState> readGroupState(const std::vector<uint8_t> &servo_ids);
